...
import { db } from "../../firebaseConfig"; // مسار الاستيراد حسب مكان ملف firebase.js
...
export default function ContactPage() {
  ...
  *رسالة جديدة من الموقع:*
  ...
  // بيانات افتراضية في حال عدم وجود بيانات من فايربيز
  const fallbackContact = {
    phone: "+966 531472119",
    email: "info@cornermak.com",
    address: "المملكة العربية السعودية-الرياض-شارع النخيل ص : ب 3362 - الرمز : 14723 - سجل تجاري 1010499776",
    workHours: {
      sundayToThursday: "9:00 صباحاً - 5:00 مساءً",
      fridaySaturday: "مغلق",
    },
  };

  const displayedContact = contactInfo || fallbackContact;
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-12 mb-16">
            ...
            <motion.div initial="hidden" animate="visible" variants={staggerContainer} className="space-y-6">
              <motion.div variants={fadeIn} className="flex items-start">
                ...
                <div>
                  ...
                  <p className="text-gray-700" dir="ltr">{displayedContact.phone}</p>
                </div>
              </motion.div>
              ...
              <motion.div variants={fadeIn} className="flex items-start">
                ...
                <div>
                  ...
                  <p className="text-gray-700">{displayedContact.email}</p>
                </div>
              </motion.div>
              ...
              <motion.div variants={fadeIn} className="flex items-start">
                ...
                <div>
                  ...
                  <p className="text-gray-700">{displayedContact.address}</p>
                </div>
              </motion.div>
              ...
              <motion.div variants={fadeIn} className="flex items-start">
                ...
                <div>
                  ...
                  <p className="text-gray-700">{displayedContact.workingHours}</p>
          ...
          >
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              {/* نموذج الإدخالات هنا نفسه كما هو */}
              <div>
                <label htmlFor="name" className="block text-sm font-medium text-gray-700 mb-1">الاسم الكامل</label>
                <input type="text" id="name" name="name" value={formData.name} onChange={handleChange} required className="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-red-500 focus:border-transparent" />
              </div>
              <div>
                <label htmlFor="email" className="block text-sm font-medium text-gray-700 mb-1">البريد الإلكتروني</label>
                <input type="email" id="email" name="email" value={formData.email} onChange={handleChange} className="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-red-500 focus:border-transparent" />
              </div>
              <div>
                <label htmlFor="phone" className="block text-sm font-medium text-gray-700 mb-1">رقم الجوال</label>
                <input type="tel" id="phone" name="phone" value={formData.phone} onChange={handleChange} className="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-red-500 focus:border-transparent" />
              </div>
              <div>
                <label htmlFor="subject" className="block text-sm font-medium text-gray-700 mb-1">الموضوع</label>
                <input type="text" id="subject" name="subject" value={formData.subject} onChange={handleChange} className="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-red-500 focus:border-transparent" />
              </div>
              <div>
                <label htmlFor="message" className="block text-sm font-medium text-gray-700 mb-1">الرسالة</label>
                <textarea id="message" name="message" value={formData.message} onChange={handleChange} rows={5} className="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-red-500 focus:border-transparent"></textarea>
              </div>
              <div>
                <motion.button type="submit" whileHover={{ scale: 1.05, y: -5 }} whileTap={{ scale: 0.95 }} transition={{ duration: 0.2 }} className="w-full px-6 py-3 bg-gradient-to-r from-red-900 to-red-700 text-white font-bold rounded-xl hover:shadow-lg transition-all duration-300">
                  إرسال الرسالة
                </motion.button>
              </div>
            </form>
